...
public class Network {
	...
	public final static int port = 57086; // port d'écoute
	...
	public String[] listen(int timeout) { // écoute sur un port pour un timeout défini (timeout 0 = infini)
		...
		try {			
			...
			String[] res = new String[strResponse.length + 2];
			res[0] = packet.getAddress().toString(); // la réponse contient en premier l'IP émettrice
			res[1] = packet.getPort() + "";
			for (int i = 2; i < strResponse.length + 2; i++) {
				res[i] = strResponse[i - 2]; // ensuite elle contient de façon scindée le message reçu
			}
			return res;
		}
	...
	public boolean sendAR(InetAddress ip, int p, String id, String s) { // envoi message avec AR
		...
		try {
			for (int i = 0; i < 5; i++) { // 5 essais pour recevoir l'AR avec 100 de timeout
				...
				if (accRec != null && accRec[4].equals("AR") && accRec[5].equals(String.valueOf(Double.valueOf(nbMsg)))) {
					nbMsg++;
					return true;
				}
			}			
